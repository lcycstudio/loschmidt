...
"""Plots of the Data """
Wlabel=['0.0','5e-14','5e-15','5e-6','5e-5']
fig = plt.figure()
ax = plt.subplot(111)
...
plt.ylabel('1st Derivative dl/dt')
plt.xlim([1.25,1.40])
ax.legend(loc = 'upper right')#bbox_to_anchor=(1.02, 1.02),ncol=1)
plt.show()
...
for num in range(len(Pfiles)):
    if num >0:
        ax.plot(tm0[1200:1400],dy[num][1200:1400],'--',label=item[num][19:-4])
    else:
        ax.plot(tm[1200:1400],dy[num][1200:1400],label='W='+Wlabel[num])
plt.xlabel('Time t')
plt.ylabel('1st Derivative dl/dt')
plt.xlim([1.25,1.40])
