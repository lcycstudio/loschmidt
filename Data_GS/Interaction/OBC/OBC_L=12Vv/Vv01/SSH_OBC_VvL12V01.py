...
textr2='\n'.join((
        r'$t_{c_1}$',))
plt.text(tc+0.2,ymin+(0-ymin)/2,textr2)#,bbox=props)
...
plt.text(2.05,2.8,'(B2)')

iax = plt.axes([-1, 0, 2, 2])
ip = InsetPosition(ax1, [0.43, 0.03, 0.4, 0.4]) #posx, posy, width, height
iax.set_axes_locator(ip)
plt.plot(tm[:600],dy2[0][:600],label='W=0.0')
for num in range(len(Pfiles)):
    plt.plot(tm[:600],dy[num][:600],label='W='+wb[num])
plt.yticks([])
plt.xticks([])
plt.show()
