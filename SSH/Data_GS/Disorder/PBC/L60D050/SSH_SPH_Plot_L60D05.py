...
"""Plot the Figure """
Wlabel=['1e-4','1e-3','1e-2','0.1','0.3','0.5','0.8','1.0']
...
ymax=0.7
plt.ylim(ymin,ymax)
plt.plot(tm,Qfiles[0],label='W=0.0')
plt.plot(tm,Qfiles[-1],':',label=item2[-1][23:-9])
for nu in range(len(Qfiles)-2,0,-1):
    ax.plot(tm,Qfiles[nu],label=item2[nu][23:-9])
...
plt.plot(tm,dy2[-1],label=item2[-1][23:-9])
for nu in range(len(Qfiles)-2,0,-1):
    plt.plot(tm,dy2[nu],label=dy2[nu][23:-9])
for num in range(len(Pfiles)):
...
plt.text(0.05,1.0,'(B2)')

iax = plt.axes([-1, 0, 2, 2])
ip = InsetPosition(ax1, [0.35, 0.02, 0.35, 0.35]) #posx, posy, width, height
iax.set_axes_locator(ip)

plt.plot(tm[100:140],dy2[0][100:140],label='W=0.0')    
plt.plot(tm[100:140],dy2[-1][100:140],':',label=dy2[-1][23:-9])
for nu in range(len(Qfiles)-2,0,-1):
    plt.plot(tm[100:140],dy2[nu][100:140],label=dy2[nu][23:-9])
for num in range(len(Pfiles)):
    plt.plot(tm[100:140],dy[num][100:140],label='W='+Wlabel[num])
    #if num==len(item)-3:
