...
textr='\n'.join((
        r'(A) L = '+str(L)+', $|\delta|$= '+str(delta),))#+' -> '+str(-delta),))
...
for num in range(len(Pfiles)):
    ax.plot(tm,Pfiles[num],label=item[num][24:-10])
plt.xlabel('t')
...
plt.text(0.05,0.9*(ymax-ymin)+ymin,'(A)')
...
ip = InsetPosition(ax1, [0.32, 0.02, 0.4, 0.4]) #posx, posy, width, height
iax.set_axes_locator(ip)
plt.plot(tm[390:420],dy2[0][390:420],label='W=0.0')
for num in range(len(Pfiles)):
    plt.plot(tm[390:420],dy[num][390:420],label=item[num][23:-10])
plt.yticks([])
plt.xticks([])
#plt.text(0.05,1.7,'(B)')
plt.show()
